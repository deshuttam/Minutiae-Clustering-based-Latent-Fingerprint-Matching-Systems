...
if display_flag == 1
  ...
  m=size(img, 1);
  n=size(img, 2);
 minutiae_img = 0 * ones(m,n,1,'uint8');
imshow(minutiae_img);
  for i=1:minu_count 
    x1 = minutiae(i, 1);
    y1 = minutiae(i, 2);
...
minutiae
ret=struct('X', minutiae_for_sc, 'TXT',  minutiae_img, 'M', minutiae, 'O', orientations, 'R', radii, 'N', neighbours, 'RO', min_o_change, 'OIMG', orient_img_m, 'OREL', o_rel); %, 'SIKP', sikp_sc); 
