...
function [ret] = extract_finger_PUF_contact(file_n)
...
[newim, binim, mask, o_rel, orient_img] =  testfin_PUF(enhimg,filename);  % testfin is from Dr. Peter Kovesi's code
[cimg, oimg2,fimg,bwimg,eimg,enhimg] =  fft_enhance_cubs(img, -1);
[newim, binim, mask, o_rel, orient_img_m] =  testfin_PUF(enhimg,filename);  % testfin is from Dr. Peter Kovesi's code
...
if display_flag == 1
  ...
  %minutiae_i = uint8((size(img, 1),size(img, 2),3)); %minutiae(minu_count, :) = [0,0];
  m=size(img, 1);
  n=size(img, 2);
 minutiae_i = 255 * ones(m,n,1,'uint8');
%imshow(minutiae_i);
  for i=1:minu_count 
    x1 = minutiae(i, 1);
    y1 = minutiae(i, 2);
...
%figure(1)
%imshow(minutiae_img)
%subplot(2,2,1), subimage(minutiae_i), title(filename)
%imshow(minutiae_i)
%name=minutiae_img;
%saveImg(name,MinutiaTerm,MinutiaBif)
  % merge thinned image and minutia_img together
  %combined = uint8(minutiae_img);
  %subplot(1,2,2), subimage(minutiae_img), title(filename)
  %Image = getframe(gcf);
%imwrite(Image.cdata, 'mask_image.jpg');
 ...
 %subplot(2,2,4), subimage(orient_img), title('Ridge orientation image ')
end

size(minutiae)
size(minutiae_for_sc)
minutiae
ret=struct('X', minutiae_for_sc, 'TXT',  minutiae_i, 'M', minutiae, 'O', orientations, 'R', radii, 'N', neighbours, 'RO', min_o_change, 'OIMG', orient_img_m, 'OREL', o_rel); %, 'SIKP', sikp_sc); 
