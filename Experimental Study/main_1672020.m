...
files = dir('E:\PhD\Matlab backup-20220704T035721Z-001\Matlab backup\Experimental Study\sc_minutia\sc_minutia\FVC2002\DB1_B\*.tif');
cd  'E:\PhD\Matlab backup-20220704T035721Z-001\Matlab backup\Experimental Study\sc_minutia\sc_minutia\';
...
if numel(BASE_IMG) ~= 0
 ...
 for i = o_start+1:5%numel(file_names)
...
for index1 = o_start+1: 5%(numel(file_names))
 ...
 BASE_IMG = char(file_a);
  ...
  CMP = strrep(char(file_b), '0',char(numel(COMP_IMG) - 6)); 
      str=(strfind(char(file_a), '_'));
  BASE = strrep(char(file_a), '0',char(numel(BASE_IMG) - 6));
